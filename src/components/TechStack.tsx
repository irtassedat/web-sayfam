...
const categoryIcons = [
  <svg key="fe" className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M6.75 7.5l3 2.25-3 2.25m4.5 0h3M4.5 19.5h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15A2.25 2.25 0 002.25 6.75v10.5A2.25 2.25 0 004.5 19.5z" /></svg>,
  <svg key="be" className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M5.25 14.25h13.5m-13.5 0a3 3 0 01-3-3m3 3a3 3 0 100 6h13.5a3 3 0 100-6m-16.5-3a3 3 0 013-3h13.5a3 3 0 013 3m-19.5 0a4.5 4.5 0 01.9-2.7L5.737 5.1a3.375 3.375 0 012.7-1.35h7.126c1.062 0 2.062.5 2.7 1.35l2.587 3.45a4.5 4.5 0 01.9 2.7" /></svg>,
  <svg key="db" className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M20.25 6.375c0 2.278-3.694 4.125-8.25 4.125S3.75 8.653 3.75 6.375m16.5 0c0-2.278-3.694-4.125-8.25-4.125S3.75 4.097 3.75 6.375m16.5 0v11.25c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125V6.375" /></svg>,
...
const categoryColors = ["#6366f1", "#10b981", "#06b6d4", "#a855f7"];

function SkillCard({ category, index, icon, color }: { category: { title: string; skills: string[] }; index: number; icon: React.ReactNode; color: string }) {
      ...
      <div className="relative z-10">
        ...
        <div className="flex flex-wrap gap-1.5">
          {category.skills.map((skill) => (
            <span key={skill} className="text-[11px] px-3 py-1.5 rounded-lg bg-surface-light text-foreground/60 border border-border/40 hover:border-primary/30 hover:text-primary transition-colors">
              {skill}
            </span>
          ))}
        </div>
  ...
  return (
    <div className="overflow-hidden py-2">
      <div
        ...
        style={{ animation: `${reverse ? "marquee-reverse" : "marquee"} ${items.length * 3}s linear infinite`, width: "max-content" }}
      >
        {doubled.map((item, i) => (
          <span key={`${item}-${i}`} className="px-4 py-2 rounded-xl glass text-xs text-muted/70 font-mono hover:text-primary transition-colors cursor-default">
            {item}
          </span>
        ))}
      </div>
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-8">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold mt-2 tracking-tight">
            {t.tech.titleA}<span className="text-gradient">{t.tech.titleB}</span>
          </h2>
        </motion.div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {t.tech.categories.map((cat, i) => (
            <SkillCard key={cat.title} category={cat} index={i} icon={categoryIcons[i]} color={categoryColors[i]} />
          ))}
        </div>
        ...
        >
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-sm font-semibold">{t.tech.awarenessTitle}</h3>
                <p className="text-[11px] text-muted">{t.tech.awarenessSub}</p>
              </div>
            </div>
            <div className="grid sm:grid-cols-3 gap-3">
              {t.tech.areas.map((area) => (
