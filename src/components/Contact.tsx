...
function ContactCard({ link, desc, index }: { link: typeof socialData[0]; desc: string; index: number }) {
  ...
  return (
    ...
    >
      <div ref={cardRef} onMouseMove={handleMouseMove} className="spotlight-card rounded-2xl p-8 text-center group glow-card h-full">
...
export default function Contact() {
  ...
  const descs = [t.contact.github, t.contact.linkedin, t.contact.email];
    ...
    <section id="contact" className="py-28 px-4 relative">
      ...
      <div className="max-w-3xl mx-auto relative z-10">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-14">
          ...
          <p className="text-sm text-muted mt-3 max-w-md mx-auto">{t.contact.subtitle}</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="mt-12 text-center">
          <div className="inline-flex items-center gap-3 px-5 py-3 rounded-2xl glass">
            ...
            <div className="text-left">
              <span className="text-xs font-medium text-foreground/80 block">{t.contact.availableTitle}</span>
              <span className="text-[10px] text-muted/60">{t.contact.availableSub}</span>
            </div>
          </div>
        </motion.div>
      </div>
